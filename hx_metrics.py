...
'''
#
# Enter local server IP address, this is IP the Flask server will run on
#
server_IP ='10.2.17.150'
server_port = '8082'
...
def get_stats():
	...
	for host in hx_creds.hosts:
		...
		# Get throughput data
		MBps_data = get_stats(host,url+uri_MBps)
		if MBps_data:
			try:
		...
		# get IOPS data
		IOPS_data = get_stats(host,url+uri_IOPS)
		if IOPS_data:
			try:
		...
		# get latency data
		Lat_data  = get_stats(host,url+uri_Lat)
		if Lat_data:
			try:
...
# calls HX API
#
def get_stats(host, url):
	logger.info("call for get_stats")
	try:
		headers = {'Connection':'close'}
		response = requests.get(url,auth=(host['username'], host['password']),headers=headers,verify=False,timeout=4)
